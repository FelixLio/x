...
export function demoPlugin(md: MarkdownIt, config: { root?: string } = {}) {
  ...
  md.renderer.render = function render(tokens, options, env: MarkdownItEnv) {
    ...
    function getCurrentHeader() {
      const levels = [...activeHeaders.keys()].toSorted((left, right) => right - left)
      const nearestLevel = levels[0]
      return nearestLevel ? activeHeaders.get(nearestLevel) : undefined
    }

    function resolveHeading(headingLevel: number, headingSlug?: string, headingTitle?: string) {
      if (headingSlug) {
    ...
    const typedTokens = tokens as typeof tokens & Array<{ type: string, tag?: string, attrs?: [string, string][] | null, content: string, children?: unknown[] }>
    ...
    typedTokens.forEach((token, index) => {
      if (token.type === 'heading_open') {
        ...
        if (headingLevel) {
          ...
          const header = resolveHeading(headingLevel, headingSlug, headingTitle)
          if (header)
