...
const role: any = {
  ai: {
    placement: 'start',
    typing: (_: any, info: any) => (info.status === 'updating' ? { effect: 'typing', step: 5, interval: 20 } : false),
    loadingRender: () => h(Flex, { align: 'center', gap: 'small' }, {
      default: () => [
        h(Spin, { size: 'small' }),
    ...
    footer: (content: string, info: any) => {
      const feedback = (info.extraInfo?.feedback as ActionsFeedbackProps['value']) || 'default'
      return h(Actions, {
        items: [
          {
            key: 'copy',
