...
const chartData = computed(() => {
  const match = content.value.match(/<custom-line[^>]*>([\s\S]*?)<\/custom-line>/)
  if (!match?.[1])
    return []
...
const chartPoints = computed(() => {
  ...
  const minX = Math.min(...chartData.value.map((item: any) => Number(item.time) || 0))
  const maxX = Math.max(...chartData.value.map((item: any) => Number(item.time) || 0))
  const minY = Math.min(...chartData.value.map((item: any) => Number(item.value) || 0))
  const maxY = Math.max(...chartData.value.map((item: any) => Number(item.value) || 0))
  ...
  return chartData.value
    .map((item: any) => {
      const xRatio = maxX === minX ? 0 : (Number(item.time) - minX) / (maxX - minX)
      const yRatio = maxY === minY ? 0 : (Number(item.value) - minY) / (maxY - minY)
...
function contentRender(value: string) {
  const normalized = value.replace(/<custom-line[^>]*>[\s\S]*?<\/custom-line>/g, '**Chart rendered below**')
...
<template>
  <Space direction="vertical" style="display: flex; width: 100%; max-height: 600px; overflow: auto;" :size="10">
