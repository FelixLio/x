...
function pickBubbleRef(instance: any) {
  ...
  if ('nativeElement' in instance)
    return instance.nativeElement ?? null
...
export const XBubbleList = defineComponent({
  ...
  setup(props, { expose, attrs }) {
    ...
    const setBubbleRef = (key: string | number) => (instance: any) => {
      ...
      if (element)
        bubbleRefs.set(key, element as HTMLElement)
      else
        bubbleRefs.delete(key)
    }

    const scrollTo: BubbleListRef['scrollTo'] = ({ key, top, behavior = 'smooth', block }) => {
      const scrollBox = scrollBoxRef.value
      if (!scrollBox)
        return
        ...
        >
          <div class={`${listPrefixCls.value}-scroll-content`}>
            {mergedItems.value.map((item) => {
              ...
              if (role === 'divider')
                return <DividerBubble {...commonProps} />

              if (role === 'system')
                return <SystemBubble {...commonProps} />

              return <Bubble {...commonProps} info={{ key, status, extraInfo }} />
            })}
          </div>
        </div>
      </div>
