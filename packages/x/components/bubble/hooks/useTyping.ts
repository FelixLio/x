...
function normalizeTypingOption(typing: boolean | BubbleAnimationOption | undefined) {
  if (!typing)
    return null
  ...
  if (typeof next.step === 'number') {
    ...
    if (min <= 0 || max <= 0 || min > max)
      throw new Error('[Bubble] `typing.step` should be a valid positive range.')
  }
...
export function useTyping(params: {
  ...
  const finish = (content: string) => {
    ...
    if (!params.streaming())
      params.onTypingComplete?.(content)
  }
  ...
  const runTyping = (reason: 'content' | 'streaming') => {
    ...
    const stepOnce = () => {
      ...
      const chunk = nextContent.slice(renderedText.length, renderedText.length + getStep(cfg.step))
